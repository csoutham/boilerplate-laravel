<div>
	<div class="flex flex-row pb-6">
		<div class="w-1/2 pb-2 pr-2 xl:pr-4 xl:pb-0 lg:w-1/4">
			@include('components.forms.internal.input', [
			   'type' => 'text',
			   'label' => __('Keywords'),
			   'name' => 'search',
		...
		<div class="w-1/2 pb-2 pr-2 xl:pr-4 xl:pb-0 lg:w-1/5">
			@include('components.forms.internal.select', [
			   'label' => __('Status'),
				'name' => 'status',
				'data' => $statuses,
				'nullValue' => 'Any',
		...
		<div class="w-full self-center hidden lg:block">
			<p class="mt-5 text-sm text-right lg:text-sm">
				Showing @if ($cars->total() != 0)
					{{ $cars->firstItem() }} to {{ $cars->lastItem() }} of
				@endif
				{{ $cars->total() }} results
			</p>
		</div>
                                ...
                                <tr class="text-left border-b border-gray-200 hover:bg-gray-50">
                                    <td class="px-4 py-4 whitespace-no-wrap">
                                        <div class="text-sm text-red-600 hover:text-black">
                                            <a href="{{ action('CarsController@edit', $car) }}">
                                                {{ $car->name }}
                                            </a>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 whitespace-no-wrap">
                                        <span
                                            class="inline-flex px-4 py-2 text-xs font-semibold text-gray-600 bg-gray-100 rounded-full">{{ ucfirst($car->status) }}</span>
                                    </td>
                                    <td class="px-4 py-4 text-sm text-gray-600 whitespace-no-wrap">
                                        {{ $car->created_at->format('d/m/Y H:i') }}
                                    </td>
                                    <td class="px-4 py-4 text-sm text-right">
                                        <form action="{{ action('CarsController@destroy', $car) }}"
                                              method="post">
                                            @csrf
                                            @method('DELETE')
